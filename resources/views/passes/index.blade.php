    ...
    var infoWindowContent = [
			@foreach ($passes as $p)
                @if(count($p->discounts) == 0)
				    ['<div class="info-box"><a href="{{ route('passes.show', $p->slug) }}" onclick="ga(\'send\', \'event\', \'Destinations-Map\', \'Image\', \'glacier\', 1);"><img src="/img/destinations/{{ $p->slug }}-315x278.jpg" style="max-width:260px; margin-bottom:16px;" alt="" /></a><h5><strong>{{ $p->name }}</strong></h5><a href="{{ route('passes.show', $p->slug) }}" class="btn btn-primary btn-sm" onclick="ga(\'send\', \'event\', \'Destinations-Map\', \'View Discounts\', \'{{ $p->slug }}\', 1);">Available May 15th <i class="icon-arrow-right"></i></a></div>'],
			    @else
