		...
		@foreach ($destinations as $d)
            @foreach ($d->passes as $p)
                @if(count($p->discounts) == 0)
				    ['<div class="info-box"><a href="{{ route('destinations.passes.show', [$d->slug, $p->slug]) }}" onclick="ga(\'send\', \'event\', \'Destinations-Map\', \'Image\', \'glacier\', 1);"><img src="/img/destinations/{{ $d->slug }}-315x278.jpg" style="max-width:260px; margin-bottom:16px;" alt="" /></a><h5><strong>{{ $d->name }}</strong></h5><a href="{{ route('destinations.passes.show',[$d->slug,$p->slug]) }}" class="btn btn-primary btn-sm" onclick="ga(\'send\', \'event\', \'Destinations-Map\', \'View Discounts\', \'{{ $p->slug }}\', 1);">Available June 1st, 2018 <i class="icon-arrow-right"></i></a></div>'],
			    @else
